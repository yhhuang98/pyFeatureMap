...
//  }
  ...
  //Calculate GLCM(s)
  int failed = 0;
  #pragma omp parallel private(bb,v) //shared(image, mask, size, strides, angles, Na, Nd, glcm, Ng,voxels,Nvox,kernelRadius,force2Ddimension)
  {
      bb = (int *)malloc(sizeof *bb * Nd * 2);
      if (!bb)
      {
      ...
      else
      {
          ...
          for (v = 0; v < Nvox; v++)
          {
            ...
            {
                failed = 1;
                /* break;  removed by hyh due to GCC error */
            }
          }
      }
      free(bb);
...
//  }
  ...
  int failed = 0;
  int partialMaxRegion = 0;
  #pragma omp parallel private(bb,v,partialMaxRegion) shared(maxRegion)  //public
  {
      maxRegion = 0;
      partialMaxRegion = 0;
...
//              return NULL;
            ...
            if (region > partialMaxRegion)
              partialMaxRegion = region;
          }
      }
      #pragma omp critical
      {
        if (partialMaxRegion > maxRegion)
          maxRegion = partialMaxRegion;
      }
      free(bb);
  }
  ...
  {
    PyErr_SetString(PyExc_IndexError, "Index overflow");
    return NULL;
  }
