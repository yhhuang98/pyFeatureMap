...
from . import generalinfo, getFeatureClasses, getImageTypes, getParameterValidationFiles, imageoperations
from .base import RadiomicsFeatureCalculationController, featureNameContained
...
class RadiomicsFeatureExtractor:
  ...
  def _applyParams(self, paramsFile=None, paramsDict=None):
    ...
    enabledFeaturesClasses = params.get('featureClass', {})
    ...
    if len(enabledFeaturesClasses) == 0:
      self.enabledFeatures = {}
      for featureClassName in self.featureClassNames:
        self.enabledFeatures[featureClassName] = []
    else:
      self.enabledFeatures = enabledFeaturesClasses
  ...
  def execute(self, imageFilepath, maskFilepath, label=None, label_channel=None, voxelBased=False,
              exportDirectory=None, parallel=False, selectedFeatureFullNames=None):
    """
    ...
    resegmentLabel = _settings.get('resegmentLabel')
    ...
    if _settings.get('resegmentRange', None) is not None:
      mask = imageoperations.resegmentMask(image, mask, **_settings)

      # Recheck to see if the mask is still valid, raises a ValueError if not
      boundingBox, correctedMask = imageoperations.checkMask(image, mask, **_settings)

      if generalInfo is not None:
        generalInfo.addMaskElements(image, mask, label, 'resegmented')
    if resegmentShape:
      shapeMask = mask
    elif resegmentLabel is not None:
      shapeMask = sitk.Cast(mask, sitk.sitkUInt8) + sitk.Cast(mask == resegmentLabel, sitk.sitkUInt8)*(label-resegmentLabel)

    if not voxelBased and parallel:
      return RadiomicsFeatureExtractor.computeFeaturesRemote.remote(image, mask,self.enabledImagetypes, self.enabledFeatures, boundingBox,
                                  shapeMask=shapeMask, selectedFeatureFullNames=selectedFeatureFullNames,
                             exportDirectory=exportDirectory, **_settings)
    else:
      return self.computeFeatures(image, mask, self.enabledImagetypes, self.enabledFeatures, boundingBox,
                                  shapeMask=shapeMask, parallel=parallel,
                                  selectedFeatureFullNames=selectedFeatureFullNames, exportDirectory=exportDirectory, **_settings)


  @staticmethod
  @ray.remote
  def computeFeaturesRemote(image, mask, enabledImagetypes, enabledFeatures, boundingBox, shapeMask=None,
                      selectedFeatureFullNames=None, parallel=False, exportDirectory=None, **kwargs):
    return RadiomicsFeatureExtractor.computeFeatures(image, mask, enabledImagetypes, enabledFeatures, boundingBox, shapeMask=shapeMask,
                      selectedFeatureFullNames=selectedFeatureFullNames, parallel=parallel, exportDirectory=exportDirectory, **kwargs)
  ...
  def loadImage(ImageFilePath, MaskFilePath, generalInfo=None, **kwargs):
    ...
    if MaskFilePath is None:
      margin = kwargs.get('kernelRadius',1)
      maskArray = numpy.ones(numpy.flip(image.GetSize())-2*margin)
      maskArray = numpy.pad(maskArray,
                               pad_width=margin,
  ...
  @staticmethod
  def computeShape(image, mask, boundingBox, enabledFeatures, selectedFeatureFullNames=None, **kwargs):
    """
    ...
    featureCalculationController = RadiomicsFeatureCalculationController(featureClasses, enabledFeatures, **kwargs)

    Nd = mask.GetDimension()
    if 'shape' in enabledFeatures.keys():
      if Nd == 3:
        featureVector.update(featureCalculationController.calculateShapeFeatures(croppedMask, 'shape', selectedFeatureFullNames))
      else:
    ...
    if 'shape2D' in enabledFeatures.keys():
      if Nd == 3:
        ...
        if not force2D:
          ...
          featureVector.update(featureCalculationController.calculateShapeFeatures(croppedMask, 'shape2D', selectedFeatureFullNames))
      elif Nd == 2:
        featureVector.update(featureCalculationController.calculateShapeFeatures(croppedMask, 'shape2D', selectedFeatureFullNames))
      else:
  ...
  @staticmethod
  def computeFeatures(image, mask, enabledImagetypes, enabledFeatures, boundingBox, shapeMask=None,
                      selectedFeatureFullNames=None, parallel=False, exportDirectory=None, **kwargs):
    r"""
    ...
    """
    logger = logging.getLogger(RadiomicsFeatureExtractor.__module__)
    featureVector = collections.OrderedDict()
    voxelBased = kwargs.get('voxelBased', False)
    kernelRadius = kwargs.get('kernelRadius', 1)

    if not voxelBased:
      if shapeMask is None:
        shapeMask = mask
      # 4. If shape descriptors should be calculated, handle it separately here
      featureVector.update(RadiomicsFeatureExtractor.computeShape(image, shapeMask, boundingBox, enabledFeatures,
                                                                  selectedFeatureFullNames=selectedFeatureFullNames,
                                                                  **kwargs))


    logger.debug('Creating image type iterator')
    imageGenerators = []
    for imageType, customKwargs in six.iteritems(enabledImagetypes):
      imageTypeFeatureName = None
      if imageType == 'LoG':
        imageTypeFeatureName = 'log-sigma'
      elif imageType == 'Wavelet':
        imageTypeFeatureName = 'wavelet'
      elif imageType == 'Original':
        imageTypeFeatureName = 'original'
      if imageTypeFeatureName is not None:
        if not featureNameContained(imageTypeFeatureName, featureFullNames = selectedFeatureFullNames, mode='partial'):
          continue
      args = kwargs.copy()
    ...
    for inputImage, imageTypeName, inputKwargs in imageGenerators:
      # print('Calculating features for %s image', imageTypeName)
      ...
      featureCalculationController.addImageType(inputImage, inputMask, imageTypeName, **inputKwargs)
      # computedFeatures = self.computeFeatures(inputImage, inputMask, imageTypeName, **inputKwargs)
    featureResults = featureCalculationController.calculateFeatures(selectedFeatureFullNames, parallel, exportDirectory)
    featureVector.update(featureResults)
  ...
  def enableFeaturesByFullName(self, featureFullNames):
    self.featureFullNames = featureFullNames
